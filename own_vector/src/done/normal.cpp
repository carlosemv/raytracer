...
float hit_sphere(const Ray& _r, const Vec3& _center, float _radius) {
	/* 
	 * Ray : A + t*B	
...
Vec3 color(const Ray& _ray) {
	...
	float t = hit_sphere(_ray, sphere_center, sphere_radius);
	if (t > 0.0) {
		Vec3 normal = (_ray.point_at(t) - sphere_center).unit_vector();
